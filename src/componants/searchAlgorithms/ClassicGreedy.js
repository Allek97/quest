...
export default function classicGreedy(
  start,
  end,
  ...
  while (!visitedSet.has(last)) {
    ...
    for (const neighbour of neighboursOfCore) {
      if (!visitedSet.has(neighbour)) {
        const heuristicNeighbour = manhattanDistance(neighbour, last);
        // const newDistance = neighbour.weight;
        const f = neighbour.weight + heuristicNeighbour; // F = G + H

        //neighbour.previousNode = core;
        //neighbour.totalDistance = newDistance;
        neighbour.heuristic = heuristicNeighbour;
        neighbour.totalDistance = core.totalDistance + neighbour.weight;
